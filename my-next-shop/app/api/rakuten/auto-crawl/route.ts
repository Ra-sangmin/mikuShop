// app/api/rakuten/auto-crawl/route.ts
// 관리자용: 아직 자식을 받아오지 않은 라쿠텐 장르를 하나 골라 줍니다.
...
export async function GET() {
  ...
  try {
    // 💡 '아직 자식 목록을 직접 받아온 적이 없는 장르'를 찾습니다 (childrenSyncedAt IS NULL).
    //    자식 행이 있는지로 판단하면, 깊은 장르를 먼저 방문해 조상만 만들어진 경우를 놓칩니다.
    ...
    const nextTarget: { genreId: number; genreName: string }[] = await prisma.$queryRaw`
      ...
      FROM RakutenCategory c
      LEFT JOIN Translation t ON t.id = c.translationId
      WHERE c.isLeaf = false AND c.isActive = true AND c.childrenSyncedAt IS NULL
