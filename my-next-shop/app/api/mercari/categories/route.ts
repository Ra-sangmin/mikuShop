// app/api/mercari/categories/route.ts
// 사용자용 메루카리 카테고리 조회.
// DB 를 먼저 쓰고, 그 카테고리를 처음 방문했을 때만(테이블이 비어 있을 때 포함) 메루카리를 기다립니다.
// 오래된 목록은 응답 후 백그라운드로 갱신됩니다. (lib/mercariCategories.ts 참고)
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      parents,
      parentName: parents[parents.length - 1]?.genreName ?? (genreId === MERCARI_ROOT_ID ? 'HOME' : ''),
      fromCache,
    });
  } catch (error) {
    ...
    console.error('Mercari Category API Error:', error);
    return NextResponse.json(
