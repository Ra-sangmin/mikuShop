...
// 🌟 app/auth/change-password 페이지와 app/mypage(내 정보) 화면의 비밀번호 변경 팝업에서
// 공용으로 쓰는 폼입니다. showHeader=false로 두면 아이콘/제목/부제 없이 입력폼만 렌더링해
// 이미 자체 제목이 있는 화면(mypage 팝업 등) 안에 자연스럽게 얹을 수 있습니다.
// onSuccess가 주어지면(팝업 등) 리다이렉트 대신 그 콜백만 호출하고, 없으면 onSuccessRedirect로 이동합니다.
...
export default function ChangePasswordForm({ showHeader = true, onSuccessRedirect = '/', onSuccess }: ChangePasswordFormProps) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (onSuccess) {
          ...
          setTimeout(() => router.push(onSuccessRedirect), 1500);
        }
      } else {
